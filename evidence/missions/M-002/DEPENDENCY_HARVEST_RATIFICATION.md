...
| Classification | `M-002 READY FOR INDEPENDENT REVIEW` |
...
Deterministic tests (`tests/contract/test_m002_validators.py`, stdlib unittest, 26 tests) prove failure for: duplicate H-ID; missing required field; invalid/missing/non-official source; unsupported decision; unsupported integration; missing/unresolved license classification; entry-count drift; BUILD without ADR; unlocked M-003 during M-002; unlocked mission without DONE dependencies; two active missions; zero active missions; DONE after active; missing dependency; dependency cycle; forward dependency reference; duplicate mission ID; DAG/register status and order desync; stale ACTIVE_MISSION pointer — and prove validity of both the historical M-001 bootstrap state and a hypothetical future M-003 REVIEW state (generalization).
...
python3 tests/contract/test_m002_validators.py        -> Ran 26 tests ... OK, exit 0
```
...
| File | Why changed |
| --- | --- |
| `06_HARVEST/OSS_HARVEST_REGISTRY.yaml` | stale M-001 lockfile-pinning policy corrected; verified license/version/rule data recorded (H-008, H-015, H-016, H-017, H-019, H-021, H-022, H-023, H-025, H-030, H-034) |
...
Old→new digests are listed in the PR description. Hashes were updated only for these five files after `sha256sum -c` confirmed they were the only mismatches; the checksum file was not mass-regenerated and no mismatch was concealed. `REPO_SEED_MANIFEST.json` is a frozen seed snapshot (M-001 precedent) and was not touched.
